                            ...
                            <div class="col-md-12"
                                ...
                                <div style="margin-top: 3px;">
                                    @foreach ($company as $item)
                                        ...
                                        <input type="checkbox" name="company-{{$item->company_id}}" style="margin: 0 5px 0 0 ;" value="{{$item->company_id}}">{{ $item->name }}</label>
                                    @endforeach
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="card">
                        <div class="stat-widget-two card-body">
                            <div class="stat-content">
                                ...
                                <div class="stat-digit showClientsCount">{{$client}}</div>
                            </div>
                            {{-- <div class="progress">
...
@section('scripts')
    ...
    <!-- 引入动画 -->
    <script src="{{ config('app.assets_path') }}/assets/minified/TweenLite.min.js"></script>
    <script type="module">
        import NumberScrollAnimate,{ ArrayDeDuplication } from "{{ config('app.assets_path') }}/assets/js/module/NumberScrolAnimation.js"
        import Message from "{{ config('app.assets_path') }}/assets/js/module/Message.js"
        //实例化一个信息提示对象
        ...
        let filterCase = (startTime,endTime) => {
            ...
            $.ajax({
                type: 'POST',
                data:data,
                url: "{{ url('api/getDataByfilterCase') }}",
        ...
        let filterClients = (startTime,endTime) => {
            ...
            $.ajax({
                type: 'POST',
                data:data,
                url: "{{ url('api/getDataByfilterClients') }}",
                success: function(data) {
                    //console.log(data)
